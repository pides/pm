...
$(function () {
    ...
    PP.Window.Iframe = !function () {
        ...
        function create(el) {
            ...
            tabchild.on({
                "click": function () {
                    toggle($(this).attr("data-id"));
                },
                "dblclick": function () {
                    var _this = $(this), index = _this.attr("data-id");
                    _this.remove();
                    contentList.find("iframe[data-id='" + index + "']").remove();
                    if(contentList.find("iframe").length == 0) contentList.children().show();
                },
        ...
        function toggle(id) {
            contentList.find("iframe").hide();
            tabList.find("li").removeClass(PP.Nav.activeCls);
            tabList.find("li[data-id='" + id + "']").addClass(PP.Nav.activeCls);
            contentList.find("iframe[data-id='" + id + "']").show();
        }
    ...
    var tabContextmenu = {
        ...
        delAll: function () {
            ...
            tabContextmenu.del("li", "iframe");
        },
        delThis: function (el) {
            if (!tabContextmenu.isInit) tabContextmenu.init();
            tabContextmenu.del("li[data-id='" + el + "']", "iframe[data-id='" + el + "']");
        },
        delRight: function (el) {
            if (!tabContextmenu.isInit) tabContextmenu.init();
            tabContextmenu.del("li[data-id]:gt(" + el + ")", "iframe[data-id]:gt(" + el + ")");
        },
        delOther: function(el){
            if (!tabContextmenu.isInit) tabContextmenu.init();
            tabContextmenu.del("li[data-id!='" + el + "']", "iframe[data-id!='" + el + "']");
        },
        del: function(bText, iText){
            tabContextmenu.bars.find(bText).remove();
            tabContextmenu.iframes.find(iText).remove();
            if(tabContextmenu.iframes.find("iframe").length == 0){
                console.log(tabContextmenu.iframes.find("iframe").length);
                tabContextmenu.iframes.children().show();
            }
        }
    };
