...
!function () {
    ...
    PP.Window.Iframe = !function(){
        ...
        function create(el){
            ...
            tabList.append('<li class="item active" data-id="'+tabNum+'"><span><label>'+el.html()+'</label></span><i class="tri"></i></li>');
            contentList.find("iframe").hide();
            contentList.append('<iframe frameborder="0" data-id="'+tabNum+'" src="'+el.attr("data-url")+'">');
            tabList.find('li').on('click', function(){
                toggle($(this), contentList);
            });
            tabList.find('li').mousedown(function(e){
                if(e.which === 3){
